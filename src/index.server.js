...
import { portals } from './DialogPortals';
...
function createPage(root, renderPortals, stateScript, styleSheet) {
  return `
    <!DOCTYPE html>
    <html lang="kr">
      <head>
...
const serverRender = async (req, res, next) => {
  ...
  const root = ReactDOMServer.renderToString(sheets.collect(jsx)); // 렌더링을 함.

  const renderPortals = portals
    .map(([children, tagId]) => {
      const portalJsx = getProviderJsx(children);
      const renderChildren = ReactDOMServer.renderToString(portalJsx);
      return `<div id=${tagId}>${renderChildren}</div>`;
    })
    .join(''); // React Portals Render

  // JSON을 문자열로 변환하고 악성 스크립트가 실행되는 것을 방지하기 위해 <를 치환 처리
  // https://redux.js.org/recipes/server-rendering#security-considerations
  const stateString = JSON.stringify(store.getState()).replace(/</g, '\\u003c');
  const stateScript = `<script>__PRELOAD_STATE__ = ${stateString}</script>`; // 리덕스 초기 상태를 스크립트로 주입.
  res.send(createPage(root, renderPortals, stateScript, sheets.toString())); // 결과물 응답.
};
